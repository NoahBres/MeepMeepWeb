 ...
 */
export class MecanumKinematics {
  /**
  ...
  static robotToWheelVelocities(
    robotVel: Pose2d,
    trackWidth: number,
    wheelBase: number = trackWidth,
    lateralMultiplier: number = 1
  ): number[] {
    const k = (trackWidth + wheelBase) / 2;
  ...
  static robotToWheelAccelerations(
    robotAccel: Pose2d,
    trackWidth: number,
    wheelBase: number = trackWidth,
    lateralMultiplier: number = 1
  ): number[] {
    return this.robotToWheelVelocities(
      robotAccel,
      trackWidth,
      wheelBase,
      lateralMultiplier
   ...
   * than 1.0 corresponds to overcompensation).
   */
  static wheelToRobotVelocities(
    wheelVelocities: number[],
    trackWidth: number,
    wheelBase: number = trackWidth,
    lateralMultiplier: number = 1
  ) {
    const k = (trackWidth + wheelBase) / 2;
    const [frontLeft, rearLeft, rearRight, frontRight] = wheelVelocities;

    return new Pose2d({
      x: wheelVelocities.reduce((acc, cur) => acc + cur, 0),
      y: (rearLeft + frontRight - frontLeft - rearRight) / lateralMultiplier,
      heading: (rearRight + frontRight - frontLeft - rearLeft) / k,
    }).times(0.25);
  }
}
