...
data class DoubleProgression(
    ...
    operator fun plus(offset: Double) =
        DoubleProgression(start + offset, step, progressionSize)

    operator fun minus(offset: Double) =
        DoubleProgression(start - offset, step, progressionSize)

    operator fun unaryMinus() = DoubleProgression(-start, -step, progressionSize)

    fun isEmpty() = progressionSize == 0
    ...
    fun split(sep: Double): Pair<DoubleProgression, DoubleProgression> {
        ...
        return when {
            ...
            sepIndex >= progressionSize -> this to DoubleProgression(sep, step, 0)
            else -> DoubleProgression(start, step, sepIndex) to
                    DoubleProgression(get(sepIndex), step, progressionSize - sepIndex)
        }
    }
    ...
    inner class IteratorImpl : Iterator<Double> {
        private val iterator: Iterator<Int> = IntRange(0, progressionSize - 1).iterator()
...
operator fun Double.minus(progression: DoubleProgression) =
    DoubleProgression(this - progression.start, -progression.step, progression.progressionSize)
