 ...
 */
@JsExport
class MotionProfile(segments: Array<MotionSegment>) {
     ...
     */
    fun reversed() = MotionProfile(segments.map { it.reversed() }.reversed().toTypedArray())

    /**
     * Returns a flipped version of the motion profile.
     */
    fun flipped() = MotionProfile(segments.map { it.flipped() }.toTypedArray())
